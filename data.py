...
class PlateDataset(Dataset):
    def __init__(self, plate_ids, strict_discard=True, load_imgs=True, load_masks=False,
                 load_chrometrics=False, load_dino=False, load_dinobloom=False, load_subcell=False):
        (self.imgs, self.info, self.masks, self.chrometrics, self.dino_zs,
         self.dinobloom_zs, self.subcell_zs) = self._load_data(plate_ids, strict_discard, load_imgs,
                                                               load_masks, load_chrometrics, load_dino,
                                                               load_dinobloom, load_subcell)
    ...
    def _load_data(self, plates, strict_discard, load_imgs, load_masks, load_chrometrics, load_dino,
                   load_dinobloom=False, load_subcell=False,
      ...
      for plate in tqdm(plates):
        ...
        if load_dinobloom:
          dinobloom_zs.append(torch.load(f'{data_dir}plate_{plate}_dinobloom_feats.pt', weights_only=False))
        if load_subcell:
          subcell_zs.append(torch.load(f'{data_dir}plate_{plate}_subcell_feats.pt', weights_only=False))
      ...
      if load_dinobloom:
         dinobloom_zs = np.concatenate(dinobloom_zs)
      if load_subcell:
         subcell_zs = np.concatenate(subcell_zs)
      if load_chrometrics:
      ...
      if strict_discard:
        ...
        if load_dinobloom:
            dinobloom_zs = dinobloom_zs[~bad_idx]
        if load_subcell:
            subcell_zs = subcell_zs[~bad_idx]
        if load_chrometrics:
      ...
      return imgs, info, masks, chrometrics, dino_zs, dinobloom_zs, subcell_zs
